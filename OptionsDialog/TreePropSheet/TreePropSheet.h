...
public:
	...
	*/
	...
	/**
	Load images for the expand/close buttons.  These will replace the
	standard +/- buttons if provided.

	@param nResId
	    The resource ID of the button images bitmap.  This must be
		in image list format with two images cells: [0]="opened"
		button, [1]="closed" button
	@param transColor
	    The background color of the bitmap, to use to generate a
		transparency mask
	@param cx
	    Width of each image cell
	*/
	void SetButtonImages(int nResId, COLORREF transColor, int cx);
	...
	*/
	...
	/**
	Get the tab control page corresponding to a given tree control item.
	*/
	int GetTabPageNum(HTREEITEM hItem);
...
protected:
	...
	*/
	virtual void RefillPageTree();

	/**
	Comparison callback for sorting the tree list.  std::sort() comparator
	semantics: returns true if a < b in sort order.
	*/

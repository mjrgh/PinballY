...
BEGIN_MESSAGE_MAP(CTreePropSheet, CPropertySheet)
	...
	ON_NOTIFY(NM_CUSTOMDRAW, s_unPageTreeId, OnCustomDraw)
END_MESSAGE_MAP()
...
void CTreePropSheet::RefillPageTree()
{
	...
	// create sorted list of the items
	struct PageDesc
	{
		PageDesc(const CString &title, int pageIndex, int nImage) : 
			title(title), pageIndex(pageIndex), nImage(nImage) { }
		CString title;
		int pageIndex;
		int nImage;
	};
	std::vector<PageDesc> pageVec;

	// populate the page list
	pageVec.reserve(nPageCount);
	for (int nPage = 0; nPage < nPageCount; ++nPage)
	{
		...
		// add it to the list
		pageVec.emplace_back(strPagePath, nPage, ti.iImage);
	}

	// Sort the list.  Even though the strings can contain path structures,
	// a simple string comparison will get the order right, since we don't
	// really care about the relative order of items that aren't at the
	// same level in the hierarchy.
	std::sort(pageVec.begin(), pageVec.end(), [this](const PageDesc &a, const PageDesc &b) {
		return TreeItemSorter(a.title, b.title);
	});

	// now insert the pages into the tree in the sorted order
	for (int i = 0; i < nPageCount; ++i)
	{
		// get the page descriptor
		const PageDesc &p = pageVec[i];

		// Create an item in the tree for the page
		HTREEITEM	hItem = CreatePageTreeItem(p.title);
		ASSERT(hItem);
		if (hItem)
		{
			m_pwndPageTree->SetItemData(hItem, p.pageIndex);
...
bool CTreePropSheet::TreeItemSorter(const CString &a, const CString &b)
{
	return lstrcmpi(a.GetString(), b.GetString()) < 0;
}
...
int CTreePropSheet::GetTabPageNum(HTREEITEM hItem)
{
	// the tab control page number is stored as the item data in the tree
	return (int)m_pwndPageTree->GetItemData(hItem);
}
...
void CTreePropSheet::UpdateCaption()
{
	...
	else if (bRealPage)
	{
		...
		if (pImages)
		{
			...
			HICON	hIcon = NULL;
			if (pTabCtrl->GetItem(GetTabPageNum(hItem), &ti))
...
BOOL CTreePropSheet::OnInitDialog() 
{
	...
	// calculate the rectangle for the tree ctrl
	...
	if (!m_bPageCaption)
	{
		...
		if ((GetStyle() & WS_CHILD) != 0 && GetParent() != 0)
...
void CTreePropSheet::OnNcDestroy()
{
	__super::OnNcDestroy();
...
void CTreePropSheet::OnPageTreeSelChanging(NMHDR *pNotifyStruct, LRESULT *plResult)
{
	...
	*plResult = 0;
	...
	// get the page for the item being activated
	NMTREEVIEW	*pTvn = reinterpret_cast<NMTREEVIEW*>(pNotifyStruct);
	HTREEITEM itemNew = pTvn->itemNew.hItem;
	int	nPage = GetTabPageNum(itemNew);

	// It's possible for a tree item to exist only as a container
	// for its children.  This happens when we have one or more
	// child pages under a common parent, and the parent doesn't
	// have its own separate dialog page (e.g., we have pages for
	// "Environment::Keyboard" and "Environment::Mouse", but no
	// page just for "Environment").  In this case, clicking on
	// the parent ("Environment") should activate the first child
	// with an actual page.
	while (nPage == -1 && itemNew != 0)
	{
		// get the new item's first child
		itemNew = m_pwndPageTree->GetChildItem(itemNew);
		if (itemNew != 0)
			nPage = GetTabPageNum(itemNew);
	}

	// switch to the new page if valid, or set a blank page if not
	BOOL bResult;
...
void CTreePropSheet::OnCustomDraw(NMHDR *pNotifyStruct, LRESULT *result)
{
	// presume we'll use the default handling
	*result = CDRF_DODEFAULT;

	// if the button images haven't been loaded, use the default drawing
	if (buttons.GetSafeHandle() == 0)
		return;

	NMTVCUSTOMDRAW *cd = (NMTVCUSTOMDRAW *)pNotifyStruct;
	...
	case CDDS_ITEMPOSTPAINT:
		// Item post paint.  Draw our custom expand/close button over
		// the default one, if the item has a button at all.
		{
			// get the tree item we're drawing - its handle is given by
			// the item spec in the notify struct
			HTREEITEM hi = (HTREEITEM)cd->nmcd.dwItemSpec;

			// Check if the item has children.  If not, there's no
			// button to draw.
			if (m_pwndPageTree->GetChildItem(hi) == 0)
				return;

			// get the item's button rectangle
			CRect rc;
			m_pwndPageTree->GetItemPartRect(hi, TVGIPR_BUTTON, &rc);

			// get the drawing context from the notify struct
			CDC *pDC = CDC::FromHandle(cd->nmcd.hdc);

			// erase the background
			CBrush br(cd->clrTextBk);
			pDC->FillRect(rc, &br);

			// figure which image to use based on the item state
			UINT state = m_pwndPageTree->GetItemState(hi, TVIS_EXPANDED);
			int iImage = (state & TVIS_EXPANDED) == TVIS_EXPANDED ? 0 : 1;

			// center the image in the button rect
			IMAGEINFO ii;
			buttons.GetImageInfo(iImage, &ii);
			SIZE szImage = { ii.rcImage.right - ii.rcImage.left, ii.rcImage.bottom - ii.rcImage.top };
			POINT pt = { (rc.left + rc.right - szImage.cx) / 2, (rc.top + rc.bottom - szImage.cy) / 2 };

			// draw the image, centered in the rectangle
			buttons.Draw(pDC, iImage, pt, 0);
...
void CTreePropSheet::SetButtonImages(int nResId, COLORREF transColor, int cx)
{
	buttons.DeleteImageList();
	buttons.Create(MAKEINTRESOURCE(nResId), cx, 1, transColor);
}

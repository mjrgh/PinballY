		...
		} else if( value_in_list(tok->c_str(), background_size_strings) )
		{
			...
			if (*tok != _t("/")) {
				...
				} else
				{
					...
					(*tok)[0] == _t('+'))
		{
			tstring posTok = *tok, sizTok;
            auto slash = posTok.find(_t('/'));
			if (found_slash && (iswdigit(posTok[0]) || posTok[0] == _t('-') || posTok[0] == _t('.') || posTok[0] == _t('+'))) {
				sizTok = posTok;
				posTok.clear();
			}
            if (!found_slash && slash != tstring::npos) {
				found_slash = true;
				sizTok = posTok.substr(slash + 1);

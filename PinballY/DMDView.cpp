...
{
	// name of the font, for matching search requests
	const TCHAR *name;
...
dmdFonts[] = {
	{ _T("20px"), &DMDFonts::Font_CC_20px_az },
	{ _T("15px"), &DMDFonts::Font_CC_15px_az },
	{ _T("12px"), &DMDFonts::Font_CC_12px_az },
	{ _T("9px"),  &DMDFonts::Font_CC_9px_az },
	{ _T("7px"),  &DMDFonts::Font_CC_7px_az },
	{ _T("5px"),  &DMDFonts::Font_CC_5px_AZ }
};
...
struct HighScoreGraphicsGenThread
{
	...
	HighScoreGraphicsGenThread(BaseView *view, DWORD seqno, RGBQUAD txtColor, const TCHAR *style) :
	...
	DWORD Main()
	{
		...
		for (auto &group : slides)
		{
			...
			auto DrawToImage = [&images, &group](int width, int height, std::function<void(Gdiplus::Graphics&)> drawFunc)
			{
				// emplace a new high-score image in the list
				images.emplace_back(HighScoreImage::NormalSpriteType, group.displayTime);
			...
			else
			{
				...
				if (fontName.size() != 0)
				{
					...
					{
						if (_tcsicmp(dmdFonts[i].name, fontName.c_str()) == 0)
						{

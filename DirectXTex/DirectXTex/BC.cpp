...
namespace
{
        ...
        DWORD flags) noexcept
    {
        ...
        for (i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
        {
            ...
            Color[i].r = static_cast<float>(floorf(Clr.r * 31.0f + 0.5f) * (1.0f / 31.0f));
            Color[i].g = static_cast<float>(floorf(Clr.g * 63.0f + 0.5f) * (1.0f / 63.0f));
            Color[i].b = static_cast<float>(floorf(Clr.b * 31.0f + 0.5f) * (1.0f / 31.0f));
...
void DirectX::D3DXDecodeBC3(XMVECTOR *pColor, const uint8_t *pBC) noexcept
{
    ...
    {
        for (int i = 1; i < 7; ++i)
            fAlpha[i + 1] = (fAlpha[0] * static_cast<float>(7 - i) + fAlpha[1] * static_cast<float>(i)) * (1.0f / 7.0f);
    }
    else
    {
        for (int i = 1; i < 5; ++i)
            fAlpha[i + 1] = (fAlpha[0] * static_cast<float>(5 - i) + fAlpha[1] * static_cast<float>(i)) * (1.0f / 5.0f);
...
void DirectX::D3DXEncodeBC3(uint8_t *pBC, const XMVECTOR *pColor, DWORD flags) noexcept
{
    ...
    for (size_t i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
    {
        ...
        fAlpha[i] = floorf(fAlph * 255.0f + 0.5f) * (1.0f / 255.0f);
    ...
    if (6 == uSteps)
    {
        ...
        for (size_t i = 1; i < 5; ++i)
            fStep[i + 1] = (fStep[0] * static_cast<float>(5 - i) + fStep[1] * static_cast<float>(i)) * (1.0f / 5.0f);
    ...
    else
    {
        ...
        for (size_t i = 1; i < 7; ++i)
            fStep[i + 1] = (fStep[0] * static_cast<float>(7 - i) + fStep[1] * static_cast<float>(i)) * (1.0f / 7.0f);

...
#pragma warning(disable : 4201)
    ...
    struct BC4_UNORM
    {
        ...
        float DecodeFromIndex(size_t uIndex) const noexcept
        {
            ...
            {
                uIndex -= 1;
                return (fred_0 * static_cast<float>(7 - uIndex) + fred_1 * static_cast<float>(uIndex)) / 7.0f;
            }
            else
            {
                if (uIndex == 6)
                ...
                if (uIndex == 7)
                    return 1.0f;
                uIndex -= 1;
                return (fred_0 * static_cast<float>(5 - uIndex) + fred_1 * static_cast<float>(uIndex)) / 5.0f;
            }
        }
